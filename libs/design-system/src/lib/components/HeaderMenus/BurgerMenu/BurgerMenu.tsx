...
export const BurgerMenu: React.FC<MenuProps> = ({ menuItems }) => {
  const [activeSubMenu, setActiveSubMenu] = React.useState<null | BurgerSubMenu>(null);
          ...
          <div className="hidden group-hover:block text-right w-fit max-w-[272px] h-fit py-4 gap-2 flex flex-col mt-2">
            {menuItems?.map((dropdownItem, dropdownItemIndex) => (
              <div
                ...
                className={cn(
                  'flex flex-col',
                  dropdownItem.border ? 'border-t-2 border-b-2 border-gray-200 mb-2' : '',
              ...
              >
                ...
                {dropdownItem.children.map((subItemChildren, subItemChildrenIndex) => (
                  <div
                  ...
                  >
                    <MenuTiles {...subItemChildren} prefix={`${subItemChildrenIndex + 1}. `} />
                  </div>
                ))}
              </div>
            ))}
