...
const WalletMovingPartSvg = ({ size }: { size: IconSize }) => (
  <>
    <svg
      width={SIZE_VALUES[size]}
      height={SIZE_VALUES[size]}
      stroke={`${STROKE_VALUES[size]}`}
      viewBox="0 0 30 30"
      fill="none"
    ...
    >
      <g fill="white">
        <path
          className="group-hover:animate-walletMovablePart1"
        ...
        />
        <path
          className="group-hover:animate-walletMovablePart2"
        ...
        />
        <circle
          className="group-hover:animate-walletMovablePart3"
          cx="12.9564"
          cy="13.3321"
          r="0.714206"
          fill=""
...
export const CustomIcon: React.FC<IconProps> = ({ name, size = 'md' }) => {
  ...
  return (
    <div className="relative hover:text-brand-600 group inline-block">
      ...
      />
      {name === 'CustomWallet' && <WalletMovingPartSvg size={size} />}
    </div>
  );
};
