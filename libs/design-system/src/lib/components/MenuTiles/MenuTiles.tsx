...
export const MenuTiles: React.FC<MenuTilesProps> = ({
  ...
  return !isDashboard ? (
    <button
      ...
      onClick={handleClick}
    >
      <div className="absolute right-0 top-0 bottom-0 bg-brand-400 w-[2px] h-4"></div>
    ...
    bg-baseBackground min-w-52 h-10 rounded-md relative group
    ${
      isActive
    ...
    >
      ...
      <div className="flex justify-start gap-2">
        {/* Text container */}
        <div className="text-right">
          <p className={`font-semibold text-sm text-gray-1000`}>{text}</p>
        </div>
      </div>
    </button>
  );
