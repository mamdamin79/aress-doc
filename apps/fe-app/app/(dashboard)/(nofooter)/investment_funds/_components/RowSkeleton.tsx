export function RowSkeleton() {
    return (
        <div className="flex h-12 items-center animate-pulse justify-start border-b mr-6 w-screen gap-2">
            <div className="h-7 w-10 rounded-md bg-gray-200"></div>
            <div className="h-7 w-16 rounded-md bg-gray-200"></div>
            <div className="w-9 h-9 mr-4 rounded-full bg-gray-200"></div>
            <div className="flex items-center gap-[115px]">
                <div className="h-3 w-20 rounded-md bg-gray-200"></div>
                <div className="h-3 w-20 rounded-md bg-gray-200"></div>
                <div className="h-3 w-20 rounded-md bg-gray-200"></div>
                <div className="h-3 w-20 rounded-md bg-gray-200"></div>
                <div className="h-3 w-20 rounded-md bg-gray-200"></div>
                <div className="h-3 w-20 rounded-md bg-gray-200"></div>
            </div>
        </div>
    )
}

