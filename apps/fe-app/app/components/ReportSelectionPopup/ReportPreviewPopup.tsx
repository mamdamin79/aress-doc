...
export const ReportPreviewPopup: React.FC<ReportSelectionPopupProps> = ({
  ...
  const [reportData, setReportData] =
    useState<FinancialReportCalculationApiModel | null>(report.data);
  useEffect(() => {
    setReportData(report.data);
  }, [report.data]);
  const { mutateAsync } = useReportsServicePostReportsByReportId();
  const [changedOptionsClone, setChangedOptionsClone] =
    useState<Record<string, OptionItem>>();
  const handleSubmit = async (changedOptions: Record<string, OptionItem>) => {
    try {
      ...
      setChangedOptionsClone(changedOptions);
      return true;
    } catch (error) {
      ...
      <div className="flex h-full w-full flex-col pt-8">
        ...
        >
          <div className="flex flex-col gap-4" dir="rtl">
            ...
            <div className="sina flex w-full justify-center py-4">
        ...
        <div className="mt-4 flex w-full flex-row justify-end gap-2 px-6 pb-4">
          ...
          <div className="w-fit">
            <Button
              ...
              mode="primary"
              size="sm"
              onClick={() => onSubmit?.(changedOptionsClone ?? {})}
            >
